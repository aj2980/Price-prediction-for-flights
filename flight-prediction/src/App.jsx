...
function FlightPricePredictor() {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center py-10 bg-gradient-to-br from-blue-100 to-transparent">
      ...
      >
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {[
            ...
            <div
              key={field.name}
              className="bg-white/70 shadow-md border border-white/30 backdrop-blur-lg rounded-xl p-4 flex flex-col transition transform hover:-translate-y-1 hover:shadow-lg"
            >
              ...
              <select
                ...
                className="mt-2 block w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500 bg-white/60 backdrop-blur-md"
              >
        ...
        <button
          type="submit"
          className="w-full py-3 text-lg font-semibold bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-xl shadow-lg hover:opacity-90 transition transform hover:-translate-y-1"
        >
          🚀 Predict Price
        </button>
      </form>
      ...
      {/* Prediction Result */}
      {prediction !== null && (
        <div className="mt-6 p-4 bg-green-200 border-l-4 border-green-600 text-green-900 rounded-xl shadow-md">
          <h2 className="text-xl font-semibold">💰 Estimated Flight Price: ₹{prediction}</h2>
        </div>
      )}
    </div>
  );
